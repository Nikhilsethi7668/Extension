...
/**
 * Scrapes Brown Boys Auto vehicles directly from their backend API
 * No browser/UI scrolling required - pure API pagination
...
export async function scrapeBrownBoysViaAPI(options = {}) {
    ...
    console.log(`[API Scraper] 🚀 Starting API-based scraping for ${targetCount} NEW vehicles`);
    ...
    const fetchPage = async (page, retryCount = 0) => {
        try {
            const url = `${API_BASE}?page=${page}&limit=${LIMIT_PER_PAGE}&keywords=`;
            console.log(`[API Scraper] 📡 Requesting page ${page}...`);
            ...
            if (retryCount < MAX_RETRIES) {
                ...
                console.error(`[API Scraper] ❌ Page ${page} failed after ${MAX_RETRIES} retries:`, error.message);
                throw error;
            }
        }
    };
