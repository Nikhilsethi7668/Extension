...
// Pull ready-to-post scraped jobs from backend and merge into local queue
async function syncReadyJobs() {
  if (!currentUser?.token || !API_CONFIG.baseUrl) return;
  try {
    const res = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.scrapeJobs}?status=ready`, {
    ...
    let changed = false;
    jobs.forEach(job => {
      const existing = postingQueue.find(q => q.id === job.id);
      if (!existing && job.scraped) {
        postingQueue.push({
          id: job.id,
          ...job.scraped,
          status: 'ready',
    ...
    if (changed) {
      await saveQueue();
      updateQueueDisplay();
      showNotification('Synced ready scraped jobs from backend', 'success');
    }
  } catch (err) {
...
async function initApiBase() {
  ...
  for (const url of candidates) {
    ...
    if (healthy) {
      ...
      if (statusDiv) {
        statusDiv.innerHTML = `✅ Connected: ${url}`;
  ...
  API_CONFIG.baseUrl = candidates[0];
  if (statusDiv) {
    statusDiv.innerHTML = '❌ Cannot reach backend (tried 3001, 3000)';
    statusDiv.style.color = '#dc3545';
  }
  showNotification('Backend unavailable. Tried ports 3001 and 3000.', 'error');
}
...
function attachEventListeners() {
  ...
  const flowCardCompose = document.getElementById('flowCardCompose');
  const flowCardQueue = document.getElementById('flowCardQueue');
  ...
  if (flowCardCompose) {
    flowCardCompose.addEventListener('click', () => switchFlow('compose'));
  }
  if (flowCardQueue) {
    flowCardQueue.addEventListener('click', () => switchFlow('queue'));
  }
...
function switchFlow(target) {
  const flows = ['compose', 'queue'];
  flows.forEach((flow) => {
    const flowCard = document.getElementById(`flowCard${flow.charAt(0).toUpperCase() + flow.slice(1)}`);
    const flowPane = document.getElementById(`flow${flow.charAt(0).toUpperCase() + flow.slice(1)}`);
    if (flowPane) {
      flowPane.classList.toggle('active', flow === target);
    }
    if (flowCard) {
      flowCard.classList.toggle('active', flow === target);
    }
  });
}
