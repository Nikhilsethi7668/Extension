...
async function testConnection() {
  ...
  try {
    ...
    statusDiv.innerHTML = '❌ Cannot connect to backend. Make sure server is running on ' + API_CONFIG.baseUrl;
...
function filterAndDisplayGallery() {
  ...
  imagesToShow = imagesToShow.filter(img => img.url !== EXCLUDED_IMAGE_URL);
  
  displayImagesGallery(imagesToShow);
}
...
async function processBatchImages() {
  ...
  if (stopBtn) {
    stopBtn.onclick = async () => {
      ...
      if (jobId) {
        try {
          await fetch(`${API_CONFIG.baseUrl}/vehicles/${window.currentVehicleId}/batch-edit-images/cancel/${jobId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-api-key': currentUser.apiKey }
          });
  ...
  try {
    ...
    // 1) Start the job — server returns immediately with a jobId
      ...
      pollInterval = setInterval(async () => {
        ...
        try {
          const statusRes = await fetch(
            `${API_CONFIG.baseUrl}/vehicles/${window.currentVehicleId}/batch-edit-images/status/${jobId}`,
            { headers: { 'x-api-key': currentUser.apiKey } }
          );
          ...
          // Update progress bar
          const messageEl = document.querySelector('#globalLoaderOverlay .loader-message');
          if (messageEl) messageEl.textContent = status.message;

          const progressFill = document.querySelector('.loader-progress-fill');
          if (progressFill) progressFill.style.width = `${status.percent || 0}%`;

          updateLoaderProgress(status.completed || 0, status.total || imagesArray.length);

          // Update individual image status items
          for (let i = 0; i < (status.completed || 0) && i < imagesArray.length; i++) {
            updateLoaderStatus(imagesArray[i], 'success', `Image ${i + 1} — Done`);
          }
          ...
          if (status.status === 'complete' || status.status === 'error') {
            ...
            if (result && result.success) {
              // Update final status for each image
              if (result.results && Array.isArray(result.results)) {
                result.results.forEach((r) => {
                  ...
                  if (r.success) {
                    ...
                    updateLoaderStatus(url, 'error', `Failed: ${r.error || 'Unknown error'}`);
                  }
                });
              }
              ...
              if (prompt) {
                chrome.storage.local.set({
                  ['prompt_' + window.currentVehicleId]: { text: prompt, id: selectedPromptId }
                });
              }
              ...
              reject(new Error((result && result.message) || 'Batch processing failed'));
            }
            return;
          }
    ...
    if (cancelled) return; // Already handled by stop button
